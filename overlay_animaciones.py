...
def overlay_animacion(video_entrada, anim_mp4, t_inicio, duracion, video_salida):
    ...
    cmd = [
        ...
        "-i", anim_mp4,
        "-filter_complex",
        f"[0:v]format=yuv420p,scale=1920:1080[base];[1:v]scale=1920:1080,setpts=PTS+{t_inicio}/TB[anim];[base][anim]overlay=0:0:enable='between(t,{t_inicio},{t_fin})'[v]",
        "-map", "[v]", "-map", "0:a",
...
def aplicar_animaciones(video_entrada, timings_json, html_folder, video_salida):
    ...
    caps = {}
    for t in timings:
        nombre_base = os.path.splitext(t["html"])[0]
        mp4_prerender = os.path.join(html_folder, nombre_base + ".mp4")
        ...
        if os.path.exists(mp4_prerender):
            ...
            if capturar_animacion(html_path, t["duration"], f"tmp_anim_{nombre_base}.mp4"):
                caps[t["html"]] = f"tmp_anim_{nombre_base}.mp4"
            else:
                print(f"   Omitiendo {t['html']}")
        else:
            print(f"   FALTA: {mp4_prerender}")
